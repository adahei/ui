...
| `UiSectionStatus` | Status indicator row with label, badges slot, and actions slot. Status states: default, changed, error, success. Supports loading and disabled states. Responsive layout via container queries. |
| `UiUserCard` | User profile card with cover image, name, description, and badges. Layouts: vertical and horizontal. Aspect ratios: portrait, square, landscape. |
...
| `UiFormField` | Wrapper for form inputs that provides label, error message, and hint text. Supports inline layout with configurable label width. Handles accessibility attributes automatically. |
...
| `UiSlider` | Range slider input with customizable min, max, step. Features: low/high value labels, value position (right or below), and custom value formatter for textual values. |
